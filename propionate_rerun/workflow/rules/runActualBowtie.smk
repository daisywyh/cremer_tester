...
rule runBowtie:
    ...
    shell:
        """
        bowtie2 --very-sensitive --end-to-end -x propionate_rerun/workflow/out/index/{params.index_name}/{params.index_name}_gene_catalogue -f -U {input.reads} -S {output}
        """
...
rule summarizeHits:
    ...
    shell:
        """
        python3 propionate_rerun/workflow/scripts/summarize_hits.py {input} {output} {wildcards.overall_pathway} {wildcards.read}
        """